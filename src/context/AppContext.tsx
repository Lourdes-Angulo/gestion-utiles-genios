...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const registrarUtil = (util: Omit<UtilEscolar, "id">) => {
  ...
  const editarUtil = (util: UtilEscolar) => {
  ...
  const desactivarUtil = (id: string) => {
    setUtiles(prev => prev.map(u => u.id === id ? { ...u, estado: u.estado === "Activo" ? "Inactivo" : "Activo" } : u));
  };
